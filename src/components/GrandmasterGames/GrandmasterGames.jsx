...
const GrandmasterGames = () => {
  const [grandmaster, setGrandmaster] = useState("magnuscarlsen"); // Default player
  ...
  const handleFetchGames = async () => {
    ...
    try {
      ...
      setGames(games.slice(0, 10)); // Show only the first 10 games
    } catch (err) {
      setError("Failed to fetch games. Please check your input or try again later.");
    }
  };
